...
class History:
    ...
    def __init__(self, master: tk.Tk, user: Usuario) -> None:
        ...
        self.history = tk.Frame(
            master,
            background="black",
            highlightthickness=0)
        self.history.pack(fill="both", expand=True)

        # Crea un scrollbar horizontal y lo posiciona, para visualizar todo el
        # contenido
        ...
        # event listener para cuando se le dé doble click, abrir el link de
        # reddit si es posible
        ...
        try:
            ...
            for entry in self.user.historial:
                ...
                if isinstance(entry[3], list):
                    self.tree.insert(parent="", index="end", iid=id, text="", values=(
                        entry[0], " - ".join((entry[1], entry[2])), entry[3][1]), tags=tag)
                else:
                    self.tree.insert(parent="", index="end", iid=id, text="", values=(
                        entry[0], " - ".join((entry[1], entry[2])), entry[3]), tags=tag)
