...
class Newsfeed:
    ...
    def create_widgets(self):
        ...
        # crear la scrollbar y el canvas contenedor que nos permitirá el desplazamiento vertical
        self.scrollbar = ttk.Scrollbar(self.newsfeed, orient="vertical")
        self.post_container = tk.Canvas(self.newsfeed, width=self.newsfeed.winfo_reqwidth()-self.scrollbar.winfo_reqwidth(), height=self.newsfeed.winfo_reqheight())

        # enlazar el scrollbar al canvas contenedor y posicionamiento de ambos en el frame contenedor del newsfeed
        ...
        # creación del frame contenedor de todos los post que posteriormente se anexaŕan
        self.post_frame = tk.Frame(self.post_container, bg = "#2255a5")

        # anexión de eventListener qjue permitirá la actualización constante de la región del desplazamiento vertical
        self.post_frame.bind("<Configure>", lambda e: self.post_container.configure(scrollregion = self.post_container.bbox("all"))) 

        # posicionamiento del frame de los post, en el canvas que permite el desplazamiento vertical
        self.post_container.create_window((0,0), window=self.post_frame, anchor="nw", width=self.post_container.winfo_reqwidth())
