...
class CreateAccount:
    ...
    def register(self):
        ...
        # Uso de Regex para comprobar la calidad de los datos para generar
        # cuentas mas seguras.

        # If que verifica si el nombre es válido, si no lo es, muestra un
        # "Warning" en la interfaz.
        if not nickname or re.search(nick_regex, nickname) is None:
            messagebox.showwarning("Warning", "Please enter a valid nickname.")
            return

        # If que verifica si la contraseña es válida, si no lo es, muestra un
        # "Warning" en la interfaz.
        if not password or re.search(password_regex, password) is None:
        ...
        if password != passwordConfirmation:
            messagebox.showwarning(
                "Warning",
                "Password confirmation is not equal to the original password.",
            )
            return

        # If que verifica si el email es válido, si no lo es, muestra un
        # "Warning" en la interfaz.
        ...
        # Si lo es, muestra una ventana emergente confirmando la creación de la
        # cuenta.
        if not new_user:
            messagebox.showwarning(
    ...
    def __init__(self, master: tk.Tk) -> None:
        ...
        self.create_account = tk.Canvas(
            ...
            relief="ridge",
            bg="black",
        )  # Crea contenedor principal tipo Canvas.
        ...
        self.background_img = tk.PhotoImage(
            file=os.path.join(
                CreateAccount.recursos_path,
                "CreateAccount.png"),
            master=self.create_account,
        )
        ...
        self.imgSignUp = tk.PhotoImage(
            file=os.path.join(CreateAccount.recursos_path, "SignUpButton.png")
        )
        ...
        self.registerButton = tk.Button(
            ...
            highlightthickness=0,
            relief="flat",
        )  # Creación del botón que llama a la funcion "register()".
        self.registerButton.place(
            x=251, y=670, width=248, height=54
        )  # Posicionamiento del Botón en el contenedor principal.
        ...
        self.imgReturn = tk.PhotoImage(
            file=os.path.join(
                CreateAccount.recursos_path,
                "ReturnToMenuButton.png"))
        self.returnButton = tk.Button(
            ...
            highlightthickness=0,
            relief="flat",
        )  # Creación del botón que llama a la funcion "returnHome()".
        self.returnButton.place(
            x=555, y=670, width=248, height=54
        )  # Posicionamiento del Botón en el contenedor principal.
        # Agrega el botón a la lista widgets.
        self.widgets.append(self.returnButton)

        # --------------------------------------------------
        # Creacion de entradas de texto de nombre, correo, contraseña y
        # confirmacion de contraseña.

        self.nicknameEntry = tk.Entry(
            bd=0, bg="#ffffff", highlightthickness=0, font=("BeVietnamPro 20")
        ...
        self.nicknameEntry.place(
            ...
            width=350,
            height=38,
        )  # Posicionamiento entrada de texto nombre.
        # Agrega la entrada de texto a la lista widgets.
        self.widgets.append(self.nicknameEntry)

        self.emailEntry2 = tk.Entry(
            bd=0, bg="#ffffff", highlightthickness=0, font=("BeVietnamPro 20")
        )  # Entrada de texto para correo.
        self.emailEntry2.place(
            x=370, y=376, width=350, height=38
        )  # Posicionamiento entrada de texto correo.
        # Agrega la entrada de texto a la lista widgets.s
        self.widgets.append(self.emailEntry2)

        self.passwordEntry = tk.Entry(
            bd=0, bg="#ffffff", highlightthickness=0, font=("BeVietnamPro 20"), show="*"
        )  # Entrada de texto para contraseña.
        self.passwordEntry.place(
            x=370, y=478, width=350, height=38
        )  # Posicionamiento entrada de texto contraseña.
        # Agrega la entrada de texto a la lista widgets.
        self.widgets.append(self.passwordEntry)

        self.passwordEntry2 = tk.Entry(
            bd=0, bg="#ffffff", highlightthickness=0, font=("BeVietnamPro 20"), show="*"
        )  # Entrada de texto para confirmar contraseña.
        self.passwordEntry2.place(
            x=370, y=585, width=350, height=38
        )  # Posicionamiento entrada de texto confirmar contraseña
        # Agrega la entrada de texto a la lista widgets.
        self.widgets.append(self.passwordEntry2)

        # Cuadro de ventana emergente para restricciones Nombre y Constraseña.
        info = [
            "Nickname: Must be at least 8 characters, can be numbers or letters.",
